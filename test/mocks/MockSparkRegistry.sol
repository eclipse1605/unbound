...
import "../../contracts/interfaces/ISparkRegistry.sol";
...
contract MockSparkRegistry is ISparkRegistry {
    ...
    function createSpark(string memory content, string memory mediaHash) external override returns (uint256) {
        uint256 sparkId = _sparkCount++;
    ...
    function deleteSpark(uint256 sparkId) external override {
    ...
    function getSpark(uint256 sparkId) external view override returns (Spark memory) {
        return _sparks[sparkId];
    }
    
    function likeSpark(uint256 sparkId) external override {
    ...
    function unlikeSpark(uint256 sparkId) external override {
    ...
    function getSparkCount() external view override returns (uint256) {
        return _sparkCount;
    }
    
    function getSparksByAuthor(address author) external view override returns (uint256[] memory) {
        return _userSparks[author];
    }
    
    function incrementReboundCount(uint256 sparkId) external override {
